...
set arc=278_sources_%datestr%_%vers%.rar

"C:\Program Files\WinRAR\WinRAR.exe" a "%arc%" *.* C:\Comp\DeaTools\*.* -r -ed -v1G -m1 -msrar;zip;jpg;jpeg;png;mp3 -x*.exe -x"*\for-client\*.*" -x"\data\" -x"*\__history\*" -x"*\__recovery\*" -xlog*.txt -x"Dcu" -x"DEXTtelegram"
